...
def main():
    ...
    def doReplace(makeExcel):
        ...
        # 参数等列表替换
        if itemDict["CANSHU"].get()==0:
            tihuanList = ["MINGCHENG","XINGHAO","CANSHU","DANJIA"]
        elif itemDict["CANSHU"].get()==1:
            tihuanList = ["MINGCHENG","XINGHAO","SHANGWUCANSHU","DANJIA"]
        elif itemDict["CANSHU"].get()==2:
            tihuanList = ["MINGCHENG","XINGHAO","SHANGWUCANSHU2","DANJIA"]
        ...
        for i in tihuanList:
            if "CANSHU" in i:
                ...
                tail = i
            r["PEIDIANXIANG_"+tail] = detailDict[key]["PEIDIANXIANG_"+i]

        # 计算LED电源数量
        r["DIANYUAN_SHULIANG"] = calc.calcDianYuan(r["BANZI_CHANG"],r["BANZI_GAO"])

        # 其他参数传递
        for head in ["CITIE","ANZHUANGFUWU","DIANYUAN",LEDLEIBIE]:
            if head == LEDLEIBIE:
                key = replaceDict["KEY"]
                #print(key)
            else:
            ...
            for j in tihuanList:
                if "CANSHU" in j:
                    ...
                    tail = j
                r[head+"_"+tail] = detailDict[key][head+"_"+j]
    ...
    for n,i in enumerate(showDict.keys()):
        label = tk.Label(mainframe,text=showDict[i]+' :').grid(row=n,column=0,sticky=tk.E) # 靠东
        if i == "CHANG" or i == "GAO":            
        ...
        if i == "CANSHU":
            v_cs = tk.IntVar()
            if i in configDict:
                v_cs.set(configDict[i])
            else:
                v_cs.set(0)
            tk.Radiobutton(mainframe,text=u"普通参数",variable=v_cs,value=0).grid(row=n, column=1)
            tk.Radiobutton(mainframe,text=u"商务参数",variable=v_cs,value=1).grid(row=n, column=2)
            tk.Radiobutton(mainframe,text=u"商务参数2",variable=v_cs,value=2).grid(row=n, column=3)
            itemDict[i]=v_cs
